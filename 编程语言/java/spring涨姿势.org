...
** DI 基于annotation注入
   ...
   #+END_SRC 
    ...
    + AnnotationConfigApplicationContext 通 过 调 用 注 解 Bean 定 义 读 取 器AnnotatedBeanDefinitionReader 的 register 方法向容器注册指定的注解 Bean， 注解 Bean 定义读取器向容器注册注解 Bean 的源码如下：
      + registerBean -->org.springframework.context.annotation.AnnotatedBeanDefinitionReader#doRegisterBean
        #+BEGIN_SRC java

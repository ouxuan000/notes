...
function block_load_html(id, count) {
	...
	var x = new Ajax();
	x.get('portal.php?mod=portalcp&ac=index&op=getblock&inajax=1&count='+count, function(s) {
...
function menuFolder(obj){
	if( obj.innerHTML=='-' ){
		obj.innerHTML = '+';
		jQuery(obj).parent().parent().next('ul').slideUp();
	}else{
		obj.innerHTML = '-';
		jQuery(obj).parent().parent().next('ul').slideDown();
	}
}
...
"    call append(line(".")+1, "\# Author: zhaozhiwei") 
"    call append(line(".")+2, "\# mail: zhao1992514@126.com") 
...
"		"后面解决expand添加字符串，不能识别命令的问题，加进特殊符号会原样输出
"		".expand(" {<CR>}<ESC>0")) 
"		ab syso System.out.println(<ESC>A;<ESC>hi
"		"这个收到下面括号自动补全的影响
"		ab try try{<ESC>jocatch(Exception e){e.printStackTrace();
"		ab main public static void main(String[] args<ESC>A{
"	endif
"	if &filetype == 'py'
...
"		"后面为什么必须加一个<CR>
"		:inoremap > <ESC>:call InsertHtmlTag()<CR>o
"		""ab 中待替换字符串不能出现<怎么破,缩写+空格样式与定义相同
"		ab script <Tab><script type="text/javascript"><BACKSPACE><BACKSPACE><BACKSPACE><ESC>o</script><ESC>kO<Tab><Tab><Tab><Tab>
"		ab style  <Tab><style type="text/css"><BACKSPACE><BACKSPACE><BACKSPACE><ESC>o</style><ESC>kO<Tab><Tab><Tab>
"		ab link <Tab><link rel="" href=""/> <ESC>ddkA<BACKSPACE><BACKSPACE><BACKSPACE><BACKSPACE><ESC>o
"		ab meta <Tab><meta name="" content=""/><ESC>ddkA<BACKSPACE><BACKSPACE><BACKSPACE><BACKSPACE><ESC>o
"	endif
"
"    "新建文件后，自动定位到文件末尾
...
:inoremap ' ''<ESC>i
:inoremap ` ``<esc>i
"标签嵌套有问题，必须改善,有html插件，这个就用不着了
"function! InsertHtmlTag()  
"	"标签匹配的正则表达式
"    let pat = '\c<\w\+\s*\(\s\+\w\+\s*=\s*[''#$;,()."a-z0-9]\+\)*\s*>'  
"    normal! a>  
"	"what? 看不懂,貌似是拿到当前行号
"    let realposition = getpos('.')  
"	"如果有匹配的数据
"    if (search(pat, 'b', realposition[1]))  
"		normal yypa/
"		""normal O
"    endif  
"	"这句不写会出很诡异的问题
"    :call cursor(realposition[1], realposition[2], realposition[3])  
"endfunction  
...
" <F8>        一键保存、编译、运行
imap <F8> <esc>:call Compile_Run_Code()<cr>
nmap <F8> :call Compile_Run_Code()<cr>
vmap <F8> <esc>:call Compile_Run_Code()<cr>
